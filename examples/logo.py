...
import lmfit
import os
from lmfitxps.models import ShirleyBG, ConvGaussianDoniachSinglett
...
data = np.genfromtxt(os.path.join(exec_dir, 'examples', 'clean_Au_4f.csv'), delimiter=',', skip_header=1)
x = data[150:-35, 0]
y = data[150:-35, 1]
...
ax.axis('off')
ax.set_xlim(91,94)
plt.savefig(os.path.join(exec_dir, 'examples', 'logo_spectrum.svg'), format='svg', bbox_inches='tight')
plt.show()